...
#
# Copyright (c) 2009-2019, some parts by Togaware Pty Ltd and other by Software AG. 
#
...
pmml.iForest <- function(model, model.name="isolationForest_Model", app.name="R", 
                         description="Isolation Forest",
                         copyright=NULL, transforms=NULL, unknownValue=NULL, anomalyThreshold=0.6, 
                         parentInvalidValueTreatment="returnInvalid", childInvalidValueTreatment="asIs",
                         ...)

{
   ...
   # requireNamespace("iForest",quietly=TRUE)
  ...
  anomalyModel <- xmlNode("AnomalyDetectionModel",attrs=c(functionName="regression", 
    algorithmType="iforest", modelName=model.name))
  anomalyModel <- append.XMLNode(anomalyModel,.pmmlMiningSchema(field, target, transforms, unknownValue,
  ...
  mmodel <- xmlNode("MiningModel",attrs=c(modelName=model.name, algorithmName="randomForest",
  ...
  segments <- lapply(1:numTrees,function(x){.makeASegment(x,model,model.name,field,target,unknownValue,childInvalidValueTreatment)})
...
.pmmlParameterList <- function(sampleSize) {
 ...
 .makeASegment <- function(b,model,model.name,field,target,unknownValue=NULL,childInvalidValueTreatment)
 { 
...
# PMML -> TreeModel -> Node
  tree <- cbind(t[,"Left"],t[,"Right"],t[,"SplitAtt"],t[,"SplitValue"],
                  t[,"Size"],t[,"AttType"],t[,"TreeDepth"])

  nodeList <- list()
  #rowId <- which(tree[,1] == max(tree[,1]))
  nodeList <- lapply(1:nrow(tree),function(x){.makeANode(x,model,tree,field)})
...
.makeANode <- function(n, mod, tinf, fieldInfo)
{
 ...
 # After the node, add the split info in pmml
     ...
     # isofor source code accounts for numeric and factor only
     # ignore logical for now 12/14/2018
     logical <- FALSE
  ...
  # split if var is numeric
     ...
     # this is always false in this implementation (1/1/2019)
     {
...
.pmmlAnomalyOutput <- function(field, target, anomalyThreshold) {
  ...
  output1 <- xmlNode("OutputField", attrs=c(name="anomalyScore", optype="continuous",
                                            dataType="double", feature="predictedValue"))
  output2 <- xmlNode("OutputField", attrs=c(name="anomaly", optype="categorical", dataType="boolean",
  ...
  #output20 <- functionToPMML("if('Predicted_value'<anomalyThreshold) {'False'} else {'True'}")
...
.pmmlAnomalyMiningOutput <- function(targetName) {
  output <- xmlNode("Output")
  output1 <- xmlNode("OutputField", attrs=c(name=paste0("Predicted_",targetName), optype="continuous",
                                            dataType="double", feature="predictedValue"))
