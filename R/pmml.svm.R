...
#' @param dataset required for one-classification only; data used to train one-class SVM model.
...
#' \code{anomalyScore} is the signed distance to the separating boundary; \code{anomalyScore} corresponds 
#' to the \code{decision.values} attribute of the output of the svm predict function in R.
#'
...
#' integer class.
#' 
#' Anomaly detection SVM models are not yet supported by DMG PMML schema version 4.3. The 
#' PMML produced by this exporter uses an extended schema (4.3Ext), and can be consumed 
#' by Zementis products.
#' 
#' @references
                     ...
                     ...)
{
  ...
  if (model$type==2) {
    ...
    xmlMiningSchema <- .pmmlMiningSchema(field=field,
    ...
    xmlOF_anomalyScore <- xmlNode("OutputField", 
                                  attrs=c(name="anomalyScore",
                                          feature="predictedValue",
                                          dataType="double",
                                          optype="continuous"))
    
    xmlOF_anomaly <- xmlNode("OutputField",
                             attrs=c(name="anomaly",feature="anomaly",
    ...
    xmlOF_svm_predict_anomaly <- xmlNode("OutputField",
                     attrs=c(name="svm_predict_anomaly",
                             feature="transformedValue",
                             dataType="boolean",
                             optype="categorical"))

    # xmlApply <- xmlNode("Apply", attrs=c('function'="*"))
    # xmlFR <-  xmlNode("FieldRef", attrs=c(field="anomalyScore"))
    # xmlConst <- xmlNode("Constant", -1) 
...
# .makeOtherNodes <- function(model,field,transforms,functionName,xmlModel,vfStart){
.makeOtherNodes <- function(model,field,transforms,functionName,xmlModel){
  ...
  if (model$type==2){vfStart <-1 } else {vfStart <- 2}
